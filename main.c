...
void handleRequest(int function, char * basic_message, fd_set *master, int fd_max, int server_socket, User **users, int client_socket,
        int *user_id);
void handleCHECK(char* msg_body, fd_set * master, User ** users, int *id, int socket);
...
void handleList(User **users, int client_socket, int userNum);
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        //client socket index
        int socket = 0;
        /*int a=FD_ISSET(0, &read_fds);
        int b=FD_ISSET(1, &read_fds);
        int c=FD_ISSET(2, &read_fds);
        int d=FD_ISSET(3, &read_fds);
        int e=FD_ISSET(4, &read_fds);
        int f=FD_ISSET(5, &read_fds);*/
        ...
        for (socket = 0; socket <= fdmax; socket++) {
            if (FD_ISSET(socket, &read_fds)) {
                ...
                char server_message[256] = "check";
                char basic_message[256];

                //receive data from client and send check upon success
...
void handleRequest(int function, char * basic_message, fd_set *master, int fdmax, int server_socket, User **users, int client_socket,
        int *user_id) {
    if (function == CHECK) {
        handleCHECK(basic_message, master, users, user_id, client_socket);
    } else
...
void handleList(User **users, int client_socket, int userNum) {
    ...
    if (s = send(client_socket, user_list, 256, 0) == -1) {
        printf("There is some error on sending list to: %d!!\n", client_socket);
    } else {
...
void handleCHECK(char* msg_body, fd_set * master, User ** users, int *id, int socket) {// check if the name exist
    char server_message[256] = "Your are new to this world!";
    char denied[256] = "Change your name and come back later";

    //If name not found, add user name to the array
    if (!findUser(users, msg_body, id)) {//if the size is 0 or can not find
